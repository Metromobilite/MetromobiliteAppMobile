...
var stylesTypes = {
	...
	covoiturage:{icon:iconeArret,iconMaxres:3,text:'LIBELLE',textMaxRes:1,titre:lang.appMobile.covoiturage},
...
function showPopupDetails(map,evt,featurePredefini){
		...
		if(element) {
			...
			if (f) {
				...
				if (appMetromobilite) {
					var rowCam = $('.popupFavori');
					if (rowCam) {
						rowCam.click(function(e){
							e.preventDefault();

							var id  = $(this).parent().attr('data-id').replace('CAM_','');
							if(listeFavoris.isCamFavorite(id)) {
								listeFavoris.removeCam(id);
								rowCam.removeClass('favori').addClass('favori-empty');
							} else {
								var lib  = $(this).parent().first().text();
								listeFavoris.addCam(id,lib);
								rowCam.removeClass('favori-empty').addClass('favori');
							}
